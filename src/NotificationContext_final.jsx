...
import { subscribeToPush, sendScheduledPush } from "./pushSubscription";
...
async function scheduleLeadReminder(lead, scheduledIds, userId) {
  ...
  for (const reminder of reminders) {
    ...
    setTimeout(async () => {
      // ✅ حفظ في Supabase (يظهر في الـ panel)
      await insertNotif(
      ...
      // ✅ إرسال Push حقيقي عن طريق Edge Function (يشتغل حتى لو الـ app مقفول)
      await sendScheduledPush(userId, reminder.title, reminder.body, reminder.key);
    }, delay);
  }
}
...
export function NotificationProvider({ children, currentUser }) {
  ...
  // ── Load on mount + Subscribe to Push ────────────────────────
  useEffect(() => {
    fetchNotifs();
    if (currentUser?.id) {
      // ✅ طلب permission وحفظ الـ subscription عند أول login
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "leads" }, async (payload) => {
        ...
        if (lead.assigned_to === currentUser.id) {
          await insertNotif(
            `New lead assigned: ${lead.name || "Unknown"}`,
            "#10b981",
            currentUser.id,
            "new_lead"
          );
          await sendScheduledPush(
            currentUser.id,
            "New Lead Assigned 🎯",
            `${lead.name || "Unknown"} has been assigned to you`,
            `new-lead-${lead.id}`
          );
        }
      })
