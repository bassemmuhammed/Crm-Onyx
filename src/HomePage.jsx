...
export default function HomePage({
  ...
  useEffect(() => {
    ...
    for (const l of leads) {
      if (l.taskDone) doneIds.add(l.id);
      if (l.taskDismissed) dismissedIds.add(`lead-${l.id}`);
    }
    setDoneLeadIds(doneIds);
  ...
  const toggleTask = id => {
    if (id.startsWith("lead-")) {
      ...
      supabase
        ...
        .then(({ error }) => {
          if (error) {
            console.error("Failed to persist task_done:", error);
            // revert على الفشل
            setDoneLeadIds(prev => {
              const next = new Set(prev);
              wasDone ? next.add(leadId) : next.delete(leadId);
              return next;
            });
          }
        });
  ...
  const deleteTask = id => {
    if (id.startsWith("lead-")) {
      ...
      // حفظ task_dismissed = true في DB
      supabase
        .from("leads")
        .update({ task_dismissed: true })
        .eq("id", leadId)
        .then(({ error }) => {
          if (error) {
            console.error("Failed to persist task_dismissed:", error);
            // revert على الفشل
            setDeletedIds(prev => {
              const next = new Set(prev);
              next.delete(id);
