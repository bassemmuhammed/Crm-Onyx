  ...
  const toggleMember = async (id) => {
    ...
    if (!newActive) {
      try {
        ...
        flash("✓ Member status updated (session revoke failed)");
      }
    } else {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          console.warn("delete-user Edge Function failed:", err);
        }
        // 3) احذف من جدول users
  ...
  const resendInvite = async (email) => {
    try {
      ...
      if (error) {
        ...
        flash("✓ Invite resent to " + email);
      }
    } catch (err) {
      flash("✗ Failed to resend invite");
    }
  };
