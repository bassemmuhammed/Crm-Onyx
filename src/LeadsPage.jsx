...
export function LeadDetailModal({ lead, open, onClose, onUpdate, salesName = "Sales", currentUserId }) {
  ...
  const handleOpenShare = useCallback(async () => {
    ...
    setShareTeam(team.filter(t => t.id !== currentUserId));
  }, [currentUserId]);

  const handleShare = useCallback(async (toUserId) => {
    if (!local?.id || !toUserId) return;
    setSharing(true);
    const result = await dbShareLead(local.id, toUserId);
    setSharing(false);
    setShareOpen(false);
    if (result) {
      // بعد المشاركة، الـ lead لم يعد معيناً لهذا المستخدم
      // نغلق الـ modal (الـ lead سيُحذف من قائمة المستخدم الحالي عبر realtime)
      onClose();
    } else {
      alert("Failed to share lead. Please try again.");
    }
  }, [local, onClose]);
        ...
        >
          ...
          >
            ...
            {sharing ? (
              ...
              <div style={{ textAlign: "center", padding: "20px 0", color: C.gray, fontSize: ".78rem" }}>
                No other sales agents available
              </div>
            ) : (
                ...
                {shareTeam.map(agent => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, textAlign: "left" }}>
                      <div style={{ fontSize: ".75rem", fontWeight: 700, color: C.white }}>{agent.name}</div>
                      <div style={{ fontSize: ".6rem", color: C.gray }}>{agent.email}</div>
                    </div>
                    <svg width="14" height="14" viewBox="0 0 256 256" fill={C.silver}><path d="M229.66,109.66l-48,48a8,8,0,0,1-11.32-11.32L204.69,112H165a88.21,88.21,0,0,0-85.23,65.31,8,8,0,0,1-15.5-4A104.06,104.06,0,0,1,165,96h39.71L170.34,62.34a8,8,0,0,1,11.32-11.32l48,48A8,8,0,0,1,229.66,109.66ZM88,208H72a40,40,0,0,1,0-80H88a8,8,0,0,0,0-16H72a56,56,0,0,0,0,112H88a8,8,0,0,0,0-16Z"/></svg>
                  </button>
                ))}
              </div>
            )}
